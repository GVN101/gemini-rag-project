...
class CeconlineSpider(CrawlSpider):
    ...
    rules = (
        ...
        Rule(LinkExtractor(allow=r'/administrators*'), callback='parse_administrations', follow=True)
    )

    def parse_administrations(self, response):
        designation = response.xpath(
            'normalize-space(//div[contains(@class, "stm-teacher__position")]/text()]/text())'
        ).get()
        cleaned_designation = self.clean_text(designation)

        staff_name = response.xpath(
            'normalize-space(//div[contains(@class, "stm-teacher__name")]/text()]/text())'
        ).get()
        cleaned_staff_name = self.clean_text(staff_name)
        return cleaned_designation, cleaned_staff_name

    def parse_page(self, response):

        self.parse_administrations(response)
